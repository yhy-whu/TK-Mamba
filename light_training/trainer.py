...
class Trainer:
    def __init__(self, env_type,
        ...
        self.num_step_per_epoch = 75 // self.num_gpus
        self.val_number = 13 // self.num_gpus
    ...
    def get_multi_processor_loader(self, train_ds, val_ds):
        ...
        data_generator = LimitedLenWrapper(self.num_step_per_epoch, data_loader=train_loader, 
                                           transform=tr_transforms,
                                             num_processes=18, num_cached=6, seeds=None,
                                             pin_memory=True, wait_time=0.02)## 设置为1否则报错，原来是num_processes=self.train_process
        if val_ds is None:
            ...
            val_data_generator = LimitedLenWrapper(self.val_number, data_loader=val_loader, transform=val_transforms,
                                                ...
                                                pin_memory=True, wait_time=0.02)##设置为1否则报错，原来是num_processes=6
    ...
    def get_inputs(self, batch):
        """
        获取输入数据，并根据数据类型决定是否包含标签（seg）。
        """
        image = batch["data"]  # 图像数据
        
        # 检查是否存在 'seg' 键，如果存在则获取标签，否则返回 None
        label = batch.get("seg", None)  # 使用 get() 方法避免 KeyError
        properties = batch.get("properties", None)  # 如果有其他属性可以一并获取

        if label is not None:
            print("处理验证数据，包含标签（seg）")
        else:
            print("处理测试数据，不包含标签（seg）")

        return image, label, properties
    
    # def print_batch_content(self,batch):
    #     for key, value in batch.items():
    #         print(f"\n==== {key} ====")
            
    #         # 如果是 Tensor，打印形状和部分数据
    #         if isinstance(value, torch.Tensor):
    #             print(f"Shape: {value.shape}")
    #             print(f"Data (partial): {value.flatten()[:10]}")  # 打印前10个元素
    ...
    def validation_single_gpu(self, val_dataset):
        ...
        for idx, batch in tqdm(enumerate(val_loader), total=len(val_loader)):
            print("批数据的键：", batch.keys())  # 打印批数据中的键名，检查是否有 'seg'
            batch = self.before_data_to_device(batch)
            batch = self.to_device(batch)
            print("处理后的键：", batch.keys())  # 检查处理后的数据是否保留了 'seg' 键
            image, label, properties = self.get_inputs(batch)
            #self.print_batch_content(batch)
