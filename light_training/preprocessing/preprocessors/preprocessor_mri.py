...
class MultiModalityPreprocessor(DefaultPreprocessor):
    ...
    # 过滤掉隐藏文件和非图像文件
        all_cases = [case for case in all_cases if not case.startswith(".") and case.endswith(".nii.gz")]
        return all_cases
    ...
    def read_data(self, case_name):
        ...
        for dfname in self.data_filenames:
            d = sitk.ReadImage(os.path.join(self.base_dir, self.image_dir, case_name))
