...
class DefaultPreprocessor(object):
    ...
    # 修改后的 get_iterable_list 方法，跳过隐藏文件和无效文件
    def get_iterable_list(self):
        # 列出所有病例，并跳过以"."开头的隐藏文件夹或无效文件
        all_cases = [
            case for case in os.listdir(os.path.join(self.base_dir, self.image_dir))
            if not case.startswith(".") and os.path.isdir(os.path.join(self.base_dir, self.image_dir, case))
        ]
    ...
    def run(self, output_spacing, 
        ...
        # 获取所有图像文件夹，并跳过隐藏文件夹或非图像文件夹
        all_iter = [case for case in self.get_iterable_list() if not case.startswith(".")]
