...
class Predictor:
    ...
    def save_to_nii(self, return_output, raw_spacing, save_dir, case_name, affine, postprocess=False):
        ...
        sitk_image = sitk.GetImageFromArray(return_output)

        if isinstance(raw_spacing[0], torch.Tensor):
            raw_spacing = [x.item() for x in raw_spacing]

        sitk_image.SetSpacing(tuple(raw_spacing))
        # 从原始图像中获取 Origin 和 Direction
        original_image = sitk.ReadImage(f"/root/autodl-tmp/data/Task10_Colon/imagesTr/{case_name}.nii.gz")
        sitk_image.SetOrigin(original_image.GetOrigin())
        sitk_image.SetDirection(original_image.GetDirection())

        output_path = os.path.join(save_dir, f"{case_name}.nii.gz")
