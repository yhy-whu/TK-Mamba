...
class RandomConcatLabeld(MapTransform):
    def __init__(self, args, keys, liver_label_list, transforms, prob=1.0):
        super().__init__(keys)
        self.args = args
        self.liver_label_list = liver_label_list  
    ...
    def __call__(self, data):
        ...
        if random.random() < self.prob:
            liver_label_path = random.choice([self.args.data_txt_path + label + ".txt" for label in self.liver_label_list])
            print(f"liver_label_path: {liver_label_path}")
            liver_label = nib.load(liver_label_path).get_fdata()  

            data_rk = {"image": image, "label": label}
            data_liver = {"image": liver_label, "label": liver_label}  

            data_rk = self.transforms(data_rk)

            data_liver = self.transforms(data_liver)

            liver_label = data_liver['label']
            liver_label[liver_label == 1] = 2  
            liver_label[liver_label == 2] = 4  

            label[liver_label == 1] = 2  
            label[liver_label == 2] = 4 
...
def get_loader_without_gt(args):
    ...
    val_txt_path = os.path.join(args.data_txt_path, "PAOT_10_val.txt")
    if not os.path.exists(val_txt_path):
        raise FileNotFoundError(f"Validation file {val_txt_path} not found!")
    data_dicts_test = []
    with open(val_txt_path, 'r') as f:
            for line in f:
                ...
                full_image_path = os.path.join(args.data_root_path, image_path)
                if not os.path.exists(full_image_path):
                    print(f"Warning: Image file {full_image_path} not found, skipping...")
                    continue
                name = image_path.split('/')[-1].split('.')[0]
                data_dicts_test.append({'image': full_image_path, 'name': name})

    print('Test len {}'.format(len(data_dicts_test)))
    if len(data_dicts_test) == 0:
        raise ValueError("No valid test data found! Please check the data paths in PAOT_10_val.txt.")
