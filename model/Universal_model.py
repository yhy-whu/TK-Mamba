...
from model.K_Mamba import K_Mamba
...
class Universal_model(nn.Module):
    def __init__(self, img_size, in_channels, out_channels, backbone = 'swinunetr', encoding = 'rand_embedding'):
        ...
        if backbone == 'swinunetr':
            ...
            self.GAP = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                torch.nn.AdaptiveAvgPool3d((1,1,1)),
                nn.Conv3d(512, 512, kernel_size=1, stride=1, padding=0)
            )
        elif backbone == 'dints':
        ...
        elif backbone == 'K_Mamba':
            self.backbone = K_Mamba(
        ...
        self.controller = nn.Conv3d(512+512, sum(weight_nums+bias_nums), kernel_size=1, stride=1, padding=0)
        if self.encoding == 'rand_embedding':
            ...
            self.text_to_vision = nn.Linear(512, 512)
    ...
    def forward(self, x_in):
        ...
        x_feat_flat = x_feat.squeeze(-1).squeeze(-1).squeeze(-1) 
        b = x_feat.shape[0]
        logits_array = []
        for i in range(b):
